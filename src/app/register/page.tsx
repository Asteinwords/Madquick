  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ email, password, twoFactorSecret }),
      });
      ...
      if (res.ok) {
        ...
        if (data.error === 'User exists') {
          toast.error('Email already registered. Please log in or use a different email.');
        } else if (data.error.includes('2FA')) {
          toast.error('Invalid 2FA setup – try regenerating.');
        } else {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="relative bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/30 p-8 transform transition-all duration-300 hover:shadow-3xl">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-slate-600 dark:text-gray-400 text-sm">Join our secure platform today</p>
          </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-slate-600 dark:text-gray-400 text-sm">
              ...
              <a href="/login" className="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-semibold transition-colors duration-200">
                Sign in
              </a>
            </p>
          </div>
